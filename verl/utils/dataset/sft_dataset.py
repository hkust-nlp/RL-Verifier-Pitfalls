    ...
    def _read_files_and_tokenize(self):
        ...
        for key in self.prompt_dict_keys:
            ...
            try:
                ...
                raise
        self.prompts = [item[0] for item in self.prompts.values.tolist()]
        ...
        for key in self.response_dict_keys:
            try:
                ...
                raise
        self.responses = [item[0] for item in self.responses.values.tolist()]

        # try to tokenize and skip those are larger than self.max_length
        skip_indices = []
        for i in range(len(self.prompts)):
            prompt = self.prompts[i]
            response = self.responses[i]
            if len(prompt + response) > 5.0 * self.max_length:
                skip_indices.append(i)
            elif len(prompt) > 3.0 * self.max_length:
                skip_indices.append(i)
        print(f'skip {len(skip_indices)} samples due to over length')
        self.prompts = [item for i, item in enumerate(self.prompts) if i not in skip_indices]
        self.responses = [item for i, item in enumerate(self.responses) if i not in skip_indices]
