...
global_executor = GlobalProcessPool.get_instance(max_workers=16, memory_limit_mb=1000)
...
def extract_solution(solution_str):
    ...
    stop_words = ["</s>", "<|im_end|>", "<|endoftext|>", "</answer>"]
...
def qwen_math_equal_subprocess(prediction, reference, timeout_seconds=10):
    ...
    try:
        ...
        result = qwen_math_equal(prediction=prediction, reference=reference, timeout=True)
        return result
    except TimeoutError:
...
def compute_score(solution_str, ground_truth, method='strict'):
    ...
    total_score = answer_accuracy + format_reward
    ...
    return {"score": total_score, "extra_info": {"format": format_reward, "answer_accuracy": answer_accuracy}}
