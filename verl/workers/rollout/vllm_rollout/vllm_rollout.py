...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, actor_module: nn.Module, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        if "mistral" in config.model_path.lower():
        ...
        elif "qwen2.5" in config.model_path.lower() or "qwen-2.5" in config.model_path.lower():
            if "7b" in config.model_path.lower():
                ...
                kwargs['stop_token_ids'] = [14582, 16141, 31198] # Question, Answer, Problem
        # else:
        #     raise NotImplementedError(f"Model {config.model_path} is not supported")
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # Check if we should use the duplicated input approach
        duplicate_prompts = self.config.get('duplicate_prompts_for_sampling', False)
        
        # Get n from sampling_params
        n_samples = getattr(self.sampling_params, 'n', 1)
        # Create a new kwargs with n set to 1 if using duplicated inputs
        gen_kwargs = kwargs.copy() if kwargs else {}
        if duplicate_prompts and n_samples > 1 and do_sample:
            gen_kwargs['n'] = 1
        ...
        with self.update_sampling_params(**gen_kwargs):
            for i in range(0, batch_size, MICRO_ROLLOUT_BATCH_SIZE):
                ...
                # Duplicate each input n times if enabled and needed
                if duplicate_prompts and n_samples > 1 and do_sample:
                    duplicated_idx_chunk = []
                    for idx_item in idx_chunk:
                        duplicated_idx_chunk.extend([idx_item] * n_samples)
                    idx_chunk = duplicated_idx_chunk
        ...
        # If we duplicated the input or using original approach with n > 1, adjust the inputs accordingly
        if n_samples > 1 and do_sample:
            if duplicate_prompts:
                # For duplicated inputs approach, we already have the responses expanded
                ...
                # Original approach using vLLM's built-in sampling
