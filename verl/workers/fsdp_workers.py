...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        self.role = role
        assert self.role in ['actor', 'rollout', 'ref', 'actor_rollout', 'actor_rollout_ref', 'reward_rollout']

        self._is_actor = self.role in ['actor', 'actor_rollout', 'actor_rollout_ref']
        self._is_rollout = self.role in ['rollout', 'actor_rollout', 'actor_rollout_ref', 'reward_rollout']
        self._is_ref = self.role in ['ref', 'actor_rollout_ref']
        self._is_reward_rollout = self.role in ['reward_rollout']
        ...
        # Also handle offload for reward_rollout
        elif self._is_reward_rollout:
            # For reward_rollout, we use rollout config for offload parameters
            self._is_offload_param = self.config.rollout.get('param_offload', False)
    ...
    def init_model(self):
        ...
        if self._is_actor or self._is_rollout:
            ...
            self.actor_module_fsdp, self.actor_optimizer, self.actor_lr_scheduler, self.actor_model_config = self._build_model_optimizer(
                ...
                role='actor' if not self._is_reward_rollout else 'ref')
...
class GenRewardModelWorker(ActorRolloutRefWorker):
    def __init__(self, config: DictConfig, role: str):
        super().__init__(config, role)
        # Load the tokenizer for the reward model
        # self.input_tokenizer = None  # the tokenizer for the input prompts
        # if config.model.input_tokenizer:
        #     from verl.utils import hf_tokenizer
        #     local_path = copy_to_local(config.model.input_tokenizer)
        #     self.input_tokenizer = hf_tokenizer(
        #         local_path, 
        #         trust_remote_code=config.model.get('trust_remote_code', False)
        #     )
        # else:
        #     # Fallback to the model's tokenizer if no specific tokenizer is provided
        #     raise ValueError("No tokenizer provided for the policy model")

    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_sequences(self, prompts: DataProto):
        # prompts = inputs.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids'])
        prompts = prompts.to('cuda')
        ...
        # breakpoint()
        # input_prompts = self._switch_chat_template(prompts)
        prompts.meta_info.update(meta_info)
        ...
        output = output.to('cpu')
        # output = self._extract_reward_score(output)
        # clear kv cache
        torch.cuda.empty_cache()
        log_gpu_memory_usage('After recompute log prob', logger=logger)
        return output
    # # Used to switch to generative reward model chat template
    # def _switch_chat_template(self, data: DataProto):
    #     """
    #     Format inputs for the generative reward model using the format:
    #     "### Reference answer: \n {ground-truth ans} ### Target answer:\n {Model's output}. \n Is the above target answer correct?"
    #     """
    #     batch_size = data.batch.batch_size[0]
        
    #     # Get the ground truth from non_tensor_batch
    #     reference_answers = [i['ground_truth'] for i in data.non_tensor_batch['reward_model']]
        
    #     # Get model outputs from responses
    #     model_outputs = []
    #     for i in range(batch_size):
    #         # Extract the response tokens
    #         prompt_ids = data.batch['responses'][i]
    ...
    #         # response_text = self.tokenizer.decode(valid_prompt_ids, skip_special_tokens=True)
    #         model_outputs.append(sequences_str)
        
    #     # Create formatted prompts for the reward model
    #     formatted_prompts = []
    #     for ref_answer, model_output in zip(reference_answers, model_outputs):
    ...
    #     max_length = self.config.generative_reward.get('max_length', 2048)
    ...
    #     # Create a new DataProto with the formatted inputs
    #     new_batch = {
    #         'input_ids': input_ids,
    ...
    #     # Preserve original meta_info
    #     meta_info = data.meta_info.copy()
        
    #     # Create a new DataProto with updated inputs
    #     new_data = DataProto(
    #         batch=TensorDict(new_batch, batch_size=batch_size),
    #         non_tensor_batch={
    #             'original_responses': model_outputs,
    ...
    # def _extract_reward_score(self, output: DataProto):
    #     """
    #     Extract reward scores from the reward model's output.
    #     Parse the output to find 'True' or 'False' and convert to reward score (1 or 0).
    #     """
    ...
    #     # Extract True/False judgments and convert to reward scores
    #     rewards = torch.zeros((batch_size, 1), dtype=torch.float32)
        
    #     for i, response in enumerate(responses):
    #         # Check for a True judgment - use case-insensitive matching and look for variants
    #         if any(marker in response.lower() for marker in ['true', 'correct', 'yes']):
    #             rewards[i, 0] = 1.0
    #         # Look for explicit False markers
    #         elif any(marker in response.lower() for marker in ['false', 'incorrect', 'no']):
    #             rewards[i, 0] = 0.0
    #         else:
    #             # If no clear True/False, use heuristic to detect reasoning 
    #             # that suggests correctness or incorrectness
    #             positive_score = sum(term in response.lower() for term in 
    #                                ['accurate', 'equivalent', 'matches', 'right', 
    #                                 'appropriate', 'valid', 'good', 'properly'])
    #             negative_score = sum(term in response.lower() for term in 
    #                                ['wrong', 'inaccurate', 'mistake', 'error', 
    #                                 'missing', 'fails', 'not correct', 'invalid'])
                
    #             # Assign reward based on balance of positive vs negative indicators
    #             if positive_score > negative_score:
    #                 rewards[i, 0] = 1.0
    #             else:
    #                 rewards[i, 0] = 0.0
        
    #     # Create token-level reward scores (one per token in the response)
    ...
    #     # Apply the reward to the last position with attention_mask=1
    #     # (representing the end of the response)
    #     attention_mask = output.batch['attention_mask'][:, -response_length:]
    #     for i in range(batch_size):
    #         # Find the last position with attention_mask=1
    #         last_positions = attention_mask[i].nonzero()
    #         if last_positions.numel() > 0:
    #             last_pos = last_positions[-1].item()
    #             token_level_scores[i, last_pos] = rewards[i, 0]
        
    #     # Add the reward scores to the output
    #     output.batch['token_level_scores'] = token_level_scores
    #     output.batch['rewards'] = rewards
        
    #     if self.rank == 0:  # Only log on rank 0 to avoid duplicate logs
    #         for i in range(min(3, batch_size)):  # Log first few examples
    #             logger.info(f"Reward Model - Response #{i}")
    #             logger.info(f"Judgment: {responses[i][:100]}...")
    #             logger.info(f"Reward: {rewards[i, 0]}")
